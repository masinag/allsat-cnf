...
class BenchAdapter:
    ...
    def to_pysmt(self) -> tuple[list[FNode], list[FNode]]:
        ...
        for node in self._circuit.topo_sort():
            args = [nodes[i] for i in sorted(self._circuit.fanin(node))]
            nodes[node] = self._node_to_pysmt(node, args)
        inputs = [nodes[node] for node in sorted(self._circuit.inputs())]
        outputs = [nodes[node] for node in sorted(self._circuit.outputs())]
