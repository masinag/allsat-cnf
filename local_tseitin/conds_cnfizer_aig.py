...
class LocalTseitinCNFizerCondsAIG(LocalTseitinCNFizer):
    ...
    def lt_pol(self, formula, count):
        ...
        if self.is_literal(left) and self.is_literal(right):
            ...
            if formula.is_or():
                # (S <-> S1 v S2)
                self.all_clauses.append(([Not(P), Not(S), S1, S2], self.guards))
                self.all_clauses.append(([Not(P), S, Not(S1)], self.guards))
                self.all_clauses.append(([Not(P), S, Not(S2)], self.guards))
                if self.verbose:
                    print("{} -> ({} <-> {} or {})".format(P, S, S1, S2))
            if formula.is_and():
                # (S <-> S1 ^ S2)
                self.all_clauses.append(([Not(P), S, Not(S1), Not(S2)], self.guards))
                self.all_clauses.append(([Not(P), Not(S), S1], self.guards))
                self.all_clauses.append(([Not(P), Not(S), S2], self.guards))
                if self.verbose:
                    print("{} -> ({} <-> {} and {})".format(P, S, S1, S2))
            if formula.is_iff():
                # (S <-> S1 <-> S2)
                self.all_clauses.append(([Not(P), Not(S), S1, Not(S2)], self.guards))
                self.all_clauses.append(([Not(P), Not(S), S2, Not(S1)], self.guards))
                self.all_clauses.append(([Not(P), S, S1, S2], self.guards))
                self.all_clauses.append(([Not(P), S, Not(S2), Not(S1)], self.guards))
                if self.verbose:
        ...
        if formula.is_or():
            ...
            self.all_clauses.append(([Not(P), Not(S), Not(S1), Not(S2)], self.guards))
            self.all_clauses.append(([Not(P), Not(S), Not(P1), Not(P2)], self.guards))

            if not leaf2:
                if self.verbose:
                    print("{} and Not({}) -> {})".format(P, S1, P2))
                    print("Not({}) -> Not({}))".format(P, P2))
                self.all_clauses.append(([Not(P), S1, P2], self.guards))
                self.all_clauses.append(([P, Not(P2)], self.guards))
            if not leaf1:
                if self.verbose:
                    print("{} and Not({}) -> {})".format(P, S2, P1))
                    print("Not({}) -> Not({}))".format(P, P1))
                self.all_clauses.append(([Not(P), S2, P1], self.guards))
        ...
        if formula.is_and():
            ...
            self.all_clauses.append(([Not(P), S, S1, S2], self.guards))
            self.all_clauses.append(([Not(P), S, Not(P1), Not(P2)], self.guards))
            ...
            if not leaf2:
                if self.verbose:
                    ...
                    print("Not({}) -> Not({}))".format(P, P2))
                self.all_clauses.append(((Not(P), Not(S1), P2), self.guards))
                self.all_clauses.append(([P, Not(P2)], self.guards))
            ...
            if not leaf1:
                if self.verbose:
                    ...
                    print("Not({}) -> Not({}))".format(P, P1))
                self.all_clauses.append(((Not(P), P1, Not(S2)), self.guards))
                self.all_clauses.append(([P, Not(P1)], self.guards))
        if formula.is_iff():
            # S <-> S1 <-> S2
            self.all_clauses.append(([Not(P), Not(S), S1, Not(S2)], self.guards))
            ...
            if not leaf1:
                #self.all_clauses.append(([P, Not(P1)], self.guards))
                self.all_clauses.append(([P1, Not(P)], self.guards))
            ...
            if not leaf2:
                #self.all_clauses.append(([P, Not(P2)], self.guards))
                self.all_clauses.append(([P2, Not(P)], self.guards))
